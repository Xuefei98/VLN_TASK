...
class VLNProcessor(DataProcessor):
    """ Processor for the GQA data set. """

    def get_train_examples(self, data_dir, file_name='R2R_train.json'):
        """ See base class."""
    ...
    def get_val_examples(self, data_dir, file_name='R2R_val_unseen.json'):
        """ See base class."""
    ...
    def get_test_examples(self, data_dir, file_name='R2R_test.json'):
        """ See base class."""
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            for j in range(len(line['path'])):
                for z in range(len(line['instructions'])):
                    guid = "%s-%s" % (set_type, str(id_num))
                    text_a = line['instructions'][z]
                    text_b = None
                    label = None if set_type.startswith('test') else 1
                    score = None if set_type.startswith('test') else 1
                    img_key = line['path'][j]
                    examples.append(InputInstance(guid=guid, text_a=text_a, text_b=text_b, label=label, score=score, img_key=img_key, q_id=None))
                    id_num = id_num + 1
        return examples
